    ...
    html.Div([
        dbc.Row([
            dbc.Col([html.Div(className="border-bold mt-2", children=[
                ...
                html.Button('Download Report', id='download-button1'),
                dcc.Download(id="download-report1")
            ...
            dbc.Col([html.Div(className="border-bold mt-2", children=[
                ...
                dcc.Graph(id='graph'),
                html.Button('Download Report', id='download-button2'),
        ...
        dbc.Row([dbc.Col([html.Div(className="border-bold mt-2", children=[
            ...
            html.Button('Download Report', id='download-button3'),
...
def update_line_fig1(selected_industry, selected_date):
    ...
    return fig1
@app.callback(Output('download-report1', 'data'), [Input('download-button1', 'n_clicks')],
              [State('industry-dropdown1', 'value'), State('date-dropdown1', 'date')])
def download_report(n_clicks, selected_industry, selected_date):
    if n_clicks is None:
        raise PreventUpdate
    ...
    csv_string = data1.to_csv(index=False)
    # Return the download link
    return dict(content=csv_string, filename='report.csv')
...
@app.callback(Output('download-report2', 'data'), [Input('download-button2', 'n_clicks')],
              [State('dropdown', 'value'), State('date-dropdown2', 'date')])
def download_report2(n_clicks, selected_dropdown_value, selected_date):
    if n_clicks is None:
        raise PreventUpdate
    ...
    # Create a CSV string with headers
    csv_string = data2.to_csv(index=False)
    # Return the download link
    return dict(content=csv_string, filename='report.csv')
...
@app.callback(Output('download-report3', 'data'), [Input('download-button3', 'n_clicks')],
              [State('industry-dropdown3', 'value'), State('date-dropdown3', 'date')])
def download_report3(n_clicks, selected_industry, selected_date):
    if n_clicks is None:
        raise PreventUpdate
    ...
    # Create a CSV string with headers
    csv_string = data3.to_csv(index=False)
    # Return the download link
    return dict(content=csv_string, filename='report.csv')
